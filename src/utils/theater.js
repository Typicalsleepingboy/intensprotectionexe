...
const parseData = (html) => {
    ...
    table.find("tbody tr").each((index, element) => {
        ...
        if (showInfoFull.includes("Show") && !showInfoFull.includes("\n")) {
            scheduleData.push({
                ...
                time // Include the show time for filtering
            });
        }
    });

    // Remove data for shows that have already passed the cutoff time
    ...
    const filteredScheduleData = scheduleData.filter(item => {
        ...
        const cutoffTime = showDateTime.add(4, 'hours'); // Add 4 hours to show time
